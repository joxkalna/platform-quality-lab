...
const parseResponse = (raw: string): { category: string; confidence: number; priority: number } => {
  ...
  return { category: parsed.category, confidence, priority: confidence > 0.8 ? 1 : 0 };
};
